...
export const useHandleMessage = () => {
  ...
  const handleMessage = useCallback(
    (message: News) => {
      const blockedWord = isBlocked(blocklist, message);

      if (blockedWord) {
        log(`[NEWSMAKER] News ommited due to blocklist (${blockedWord})`);
        return;
      }
      ...
      if (!exists) {
        ...
        const newId = 'id' in message ? message.id : genId();
        const obj = { id: newId, news, symbols, receivedAt: Date.now() };
