...
import { OrderSide, OrderType } from 'gvm-cex/dist/types';
import { adjust } from 'gvm-cex/dist/utils/safe-math';

import { balanceAtom, selectedMarketAtom } from '../../app-state';
import {
  tradeSideAtom,
  priceScaleRatioAtom,
  nbOrdersAtom,
...
export const scaledInRiskTradeAtom = atom((get) => {
  ...
  const entryQty = adjust(rawEntrySize, qtyStepSize);
  const scaledInQty = adjust(rawScaleSize, qtyStepSize);
  const totalQty = adjust(totalSize, qtyStepSize);
  ...
  const orders: Array<{
    ...
    size: adjust(scaledSize, qtyStepSize),
    price: adjust(price, tickSize),
