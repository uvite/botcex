...
import type { OrderSide, PlaceOrderOpts } from 'gvm-cex/dist/types';
...
export const useTwapTrade = () => {
  ...
  const startTwap = async (opts: TWAPOpts) => {
    ...
    const updateState = (state: TWAPState) => {
      setTwaps((t) =>
        t.map((t2) => (t2.id === newId ? { ...t2, ...state } : t2))
      );
    };

    const twap = new TWAPManager(opts, updateState, placeOrders);

    setTwaps((t) => [...t, { ...twap.state, id: newId }]);
    twapInstances.set(newId, twap);
  ...
  const pauseTwap = (twapId: string) => {
    const twap = twapInstances.get(twapId);
    if (twap) twap.pause();
  };

  const resumeTwap = (twapId: string) => {
    const twap = twapInstances.get(twapId);
    if (twap) twap.resume();
  };

  const stopTwap = (twapId: string) => {
    const twap = twapInstances.get(twapId);
    if (twap) {
      twap.stop();
      twapInstances.delete(twapId);

      setTwaps((t) => {
        const next = t.filter((t2) => t2.id !== twapId);
