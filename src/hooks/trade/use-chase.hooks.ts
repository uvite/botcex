...
import type { Order, Position, StoreData } from 'gvm-cex/dist/types';
import {
  OrderTimeInForce,
  OrderType,
  OrderSide,
  PositionSide,
} from 'gvm-cex/dist/types';
...
export class ChaseManager {
  ...
  onExchangeStoreUpdate = (update: StoreData) => {
...
export const useChaseTrade = () => {
  ...
  const startChase = async (opts: ChaseOpts) => {
    if (exchange) {
      const chase = new ChaseManager(genId(), opts, exchange, fatFinger);
