...
import type { PlaceOrderOpts, Writable } from 'gvm-cex/dist/types';
...
export const aioTradeAtom = atom((get) => {
  ...
  const ticker = get(selectedTickerAtom);
  const market = get(selectedMarketAtom);

  const totalQuantity = adjust(
    size / (ticker?.last ?? 0),
  ...
  const orders: Array<Writable<PlaceOrderOpts>> = [];
