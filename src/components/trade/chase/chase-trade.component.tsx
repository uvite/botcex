...
import { adjust, afterDecimal } from 'gvm-cex/dist/utils/safe-math';
...
export const ChaseTradeComponent = () => {
  ...
  const min = adjust(price - (price * limit) / 100, pPrice).toFixed(dCount);
  const max = adjust(price + (price * limit) / 100, pPrice).toFixed(dCount);
